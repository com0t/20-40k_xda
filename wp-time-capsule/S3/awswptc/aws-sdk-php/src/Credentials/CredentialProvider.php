 ...
 * promise that is fulfilled with an {@see \AwsWPTC\Credentials\CredentialsInterface}
 * or rejected with an {@see \AwsWPTC\Exception\CredentialsException}.
 *
 * <code>
 ...
 * {@see AwsWPTC\Credentials\CredentialProvider::chain}. This function accepts
...
class CredentialProvider
{
     ...
     * then check for credential_process in the "default" profile in ~/.awswptc/credentials,
     * then checks for the "default" profile in ~/.awswptc/credentials,
     * then for credential_process in the "default profile" profile in ~/.awswptc/config,
     * then checks for "profile default" profile in ~/.awswptc/config (which is
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (
            ...
            $defaultChain['sso'] = self::sso(
                'profile default',
                self::getHomeDir() . '/.awswptc/config',
                $config
            );
     ...
     * instance of AwsWPTC\CacheInterface. Forwards calls when no credentials found
     ...
     * @see AwsWPTC\Credentials\InstanceProfileProvider for $config details.
     */
    ...
    {
        $filename = $filename ?: (self::getHomeDir() . '/.awswptc/config');
        ...
        return function () use ($ssoProfileName, $filename, $config) {
            ...
            if (empty($config['ssoClient'])) {
                $ssoClient = new AwsWPTC\SSO\SSOClient([
     ...
     * @see AwsWPTC\Credentials\EcsCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see AwsWPTC\Credentials\AssumeRoleCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see AwsWPTC\Credentials\AssumeRoleWithWebIdentityCredentialProvider for
     * $config details.
     */
     ...
     *                              the "default" profile in "~/.awswptc/credentials".
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use ($profile, $filename) {
            ...
            $data = \AwsWPTC\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: $filename");
            }
            if (!isset($data[$profile])) {
    ...
    {
        $profileData = \AwsWPTC\parse_ini_file($filename, true, INI_SCANNER_RAW);

        // If loading .awswptc/credentials, also load .awswptc/config when AWS_SDK_LOAD_NONDEFAULT_CONFIG is set
        if ($filename === self::getHomeDir() . '/.awswptc/credentials'
            && getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')
        ) {
            $configFilename = self::getHomeDir() . '/.awswptc/config';
            $configProfileData = \AwsWPTC\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    /**
     * Gets profiles from ~/.awswptc/credentials and ~/.awswptc/config ini files
     */
    private static function loadDefaultProfiles() {
        $profiles = [];
        $credFile = self::getHomeDir() . '/.awswptc/credentials';
        $configFile = self::getHomeDir() . '/.awswptc/config';
        if (file_exists($credFile)) {
            $profiles = \AwsWPTC\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }

        if (file_exists($configFile)) {
            $configProfileData = \AwsWPTC\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
    ...
    private static function getFileName($filename)
    {
        if (!isset($filename)) {
            $filename = getenv(self::ENV_SHARED_CREDENTIALS_FILE) ?:
                (self::getHomeDir() . '/.awswptc/credentials');
        }
        return $filename;
    }
}
