...
class Tokenizer
{
    ...
    {
        $this->textMode = $textmode & (\WP_Typography\Vendor\Masterminds\HTML5\Elements::TEXT_RAW | \WP_Typography\Vendor\Masterminds\HTML5\Elements::TEXT_RCDATA);
    ...
    protected function consumeData()
    {
        ...
        if (\false === $tok) {
            ...
            switch ($this->textMode) {
                case \WP_Typography\Vendor\Masterminds\HTML5\Elements::TEXT_RAW:
                    $this->rawText($tok);
                    break;
                case \WP_Typography\Vendor\Masterminds\HTML5\Elements::TEXT_RCDATA:
                    $this->rcdata($tok);
                    break;
                default:
                    if ('<' === $tok || '&' === $tok) {
    ...
    protected function rcdata($tok)
    {
        ...
        $sequence = '</' . $this->untilTag;
        $txt = '';
        $caseSensitive = !\WP_Typography\Vendor\Masterminds\HTML5\Elements::isHtml5Element($this->untilTag);
    ...
    protected function tagName()
    {
        ...
        } catch (\WP_Typography\Vendor\Masterminds\HTML5\Parser\ParseError $e) {
            $selfClose = \false;
        }
    ...
    protected function attribute(&$attributes)
    {
        ...
        if ('<' == $tok) {
            ...
            throw new \WP_Typography\Vendor\Masterminds\HTML5\Parser\ParseError('Start tag inside of attribute.');
        }
    ...
    protected function comment()
    {
        ...
        if ("\0" == $tok) {
            $tok = \WP_Typography\Vendor\Masterminds\HTML5\Parser\UTF8Utils::FFFD;
        }
    ...
    protected function doctype()
    {
        ...
        // EOF: die.
        if (\false === $tok) {
            $this->events->doctype('html5', \WP_Typography\Vendor\Masterminds\HTML5\Parser\EventHandler::DOCTYPE_NONE, '', \true);
            $this->eof();
            return \true;
        }
        // NULL char: convert.
        ...
        $doctypeName = \strtolower(\strtr($doctypeName, "\0", \WP_Typography\Vendor\Masterminds\HTML5\Parser\UTF8Utils::FFFD));
        $tok = $this->scanner->current();
        // If false, emit a parse error, DOCTYPE, and return.
        if (\false === $tok) {
            $this->parseError('Unexpected EOF in DOCTYPE declaration.');
            $this->events->doctype($doctypeName, \WP_Typography\Vendor\Masterminds\HTML5\Parser\EventHandler::DOCTYPE_NONE, null, \true);
            return \true;
        }
        ...
        if (('PUBLIC' == $pub || 'SYSTEM' == $pub) && $white > 0) {
            // Get the sys ID.
            $type = 'PUBLIC' == $pub ? \WP_Typography\Vendor\Masterminds\HTML5\Parser\EventHandler::DOCTYPE_PUBLIC : \WP_Typography\Vendor\Masterminds\HTML5\Parser\EventHandler::DOCTYPE_SYSTEM;
    ...
    protected function decodeCharacterReference($inAttribute = \false)
    {
        ...
        if ('#' === $tok) {
            ...
            if ('x' === $tok || 'X' === $tok) {
                ...
                $entity = \WP_Typography\Vendor\Masterminds\HTML5\Parser\CharacterReference::lookupHex($hex);
            } else {
                ...
                $entity = \WP_Typography\Vendor\Masterminds\HTML5\Parser\CharacterReference::lookupDecimal($numeric);
            }
            ...
            $entity = \WP_Typography\Vendor\Masterminds\HTML5\Parser\CharacterReference::lookupName($cname);
