 ...
 * parser.
 */
class DOMTreeBuilder implements \WP_Typography\Vendor\Masterminds\HTML5\Parser\EventHandler
{
    /**
    ...
    public function __construct($isFragment = \false, array $options = array())
    {
        ...
        $this->rules = new \WP_Typography\Vendor\Masterminds\HTML5\Parser\TreeBuildingRules();
        $implicitNS = array();
     ...
     */
    public function setInstructionProcessor(\WP_Typography\Vendor\Masterminds\HTML5\InstructionProcessor $proc)
    {
        $this->processor = $proc;
    }
    ...
    public function startTag($name, $attributes = array(), $selfClosing = \false)
    {
        ...
        if ($this->insertMode >= static::IM_IN_BODY && \WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($name, \WP_Typography\Vendor\Masterminds\HTML5\Elements::AUTOCLOSE_P)) {
            $this->autoclose('p');
        }
        // Set insert mode:
        switch ($name) {
        ...
        if ($this->insertMode === static::IM_IN_SVG) {
            $lname = \WP_Typography\Vendor\Masterminds\HTML5\Elements::normalizeSvgElement($lname);
        }
        $pushes = 0;
        ...
        if ($this->onlyInline && \WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($lname, \WP_Typography\Vendor\Masterminds\HTML5\Elements::BLOCK_TAG)) {
        ...
        if (\WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($lname, \WP_Typography\Vendor\Masterminds\HTML5\Elements::BLOCK_ONLY_INLINE)) {
            $this->onlyInline = $lname;
        }
        // When we add some namespacess, we have to track them. Later, when "endElement" is invoked, we have to remove them.
        // When we are on a void tag, we do not need to care about namesapce nesting.
        if ($pushes > 0 && !\WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($name, \WP_Typography\Vendor\Masterminds\HTML5\Elements::VOID_TAG)) {
        ...
        foreach ($attributes as $aName => $aVal) {
            ...
            if ($this->insertMode === static::IM_IN_SVG) {
                $aName = \WP_Typography\Vendor\Masterminds\HTML5\Elements::normalizeSvgAttribute($aName);
            } elseif ($this->insertMode === static::IM_IN_MATHML) {
                $aName = \WP_Typography\Vendor\Masterminds\HTML5\Elements::normalizeMathMlAttribute($aName);
            }
            try {
        ...
        if ($this->frag !== $this->current && $this->rules->hasRules($name)) {
            ...
            if (!\WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($name, \WP_Typography\Vendor\Masterminds\HTML5\Elements::VOID_TAG)) {
                $this->current = $ele;
            }
            ...
            if (\WP_Typography\Vendor\Masterminds\HTML5\Elements::isHtml5Element($name)) {
                $selfClosing = \false;
            }
        }
        ...
        return \WP_Typography\Vendor\Masterminds\HTML5\Elements::element($name);
    }
    public function endTag($name)
    {
        ...
        if ('br' === $name) {
            ...
            $this->startTag('br');
        } elseif (\WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($name, \WP_Typography\Vendor\Masterminds\HTML5\Elements::VOID_TAG)) {
            return;
        }
    ...
    public function processingInstruction($name, $data = null)
    {
        ...
        if ($this->processor instanceof \WP_Typography\Vendor\Masterminds\HTML5\InstructionProcessor) {
