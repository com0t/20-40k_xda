 ...
 */
class OutputRules implements \WP_Typography\Vendor\Masterminds\HTML5\Serializer\RulesInterface
{
    /**
    ...
    public function setTraverser(\WP_Typography\Vendor\Masterminds\HTML5\Serializer\Traverser $traverser)
    {
    ...
    public function element($ele)
    {
        ...
        if ('svg' == $name) {
            ...
            $name = \WP_Typography\Vendor\Masterminds\HTML5\Elements::normalizeSvgElement($name);
        } elseif ('math' == $name) {
            $this->outputMode = static::IM_IN_MATHML;
        }
        $this->openTag($ele);
        if (\WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($name, \WP_Typography\Vendor\Masterminds\HTML5\Elements::TEXT_RAW)) {
    ...
    {
        if (isset($ele->parentNode) && isset($ele->parentNode->tagName) && \WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($ele->parentNode->localName, \WP_Typography\Vendor\Masterminds\HTML5\Elements::TEXT_RAW)) {
            $this->wr($ele->data);
            return;
        }
    ...
    protected function attrs($ele)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            ...
            if ($this->outputMode == static::IM_IN_SVG) {
                $name = \WP_Typography\Vendor\Masterminds\HTML5\Elements::normalizeSvgAttribute($name);
            } elseif ($this->outputMode == static::IM_IN_MATHML) {
                $name = \WP_Typography\Vendor\Masterminds\HTML5\Elements::normalizeMathMlAttribute($name);
            }
            $this->wr(' ')->wr($name);
    ...
    protected function enc($text, $attribute = \false)
    {
        ...
        if ($this->hasHTML5) {
            ...
            return \strtr($text, \WP_Typography\Vendor\Masterminds\HTML5\Serializer\HTML5Entities::$map);
        }
    }
    /**
     * Escape test.
