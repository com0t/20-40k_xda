...
class HTML5
{
    ...
    public function parse($input, array $options = array())
    {
        ...
        $events = new \WP_Typography\Vendor\Masterminds\HTML5\Parser\DOMTreeBuilder(\false, $options);
        $scanner = new \WP_Typography\Vendor\Masterminds\HTML5\Parser\Scanner($input, !empty($options['encoding']) ? $options['encoding'] : 'UTF-8');
        $parser = new \WP_Typography\Vendor\Masterminds\HTML5\Parser\Tokenizer($scanner, $events, !empty($options['xmlNamespaces']) ? \WP_Typography\Vendor\Masterminds\HTML5\Parser\Tokenizer::CONFORMANT_XML : \WP_Typography\Vendor\Masterminds\HTML5\Parser\Tokenizer::CONFORMANT_HTML);
        $parser->parse();
    ...
    {
        $options = \array_merge($this->defaultOptions, $options);
        $events = new \WP_Typography\Vendor\Masterminds\HTML5\Parser\DOMTreeBuilder(\true, $options);
        $scanner = new \WP_Typography\Vendor\Masterminds\HTML5\Parser\Scanner($input, !empty($options['encoding']) ? $options['encoding'] : 'UTF-8');
        $parser = new \WP_Typography\Vendor\Masterminds\HTML5\Parser\Tokenizer($scanner, $events, !empty($options['xmlNamespaces']) ? \WP_Typography\Vendor\Masterminds\HTML5\Parser\Tokenizer::CONFORMANT_XML : \WP_Typography\Vendor\Masterminds\HTML5\Parser\Tokenizer::CONFORMANT_HTML);
        $parser->parse();
        $this->errors = $events->getErrors();
    ...
    public function save($dom, $file, $options = array())
    {
        ...
        $options = \array_merge($this->defaultOptions, $options);
        $rules = new \WP_Typography\Vendor\Masterminds\HTML5\Serializer\OutputRules($stream, $options);
        $trav = new \WP_Typography\Vendor\Masterminds\HTML5\Serializer\Traverser($dom, $stream, $rules, $options);
        $trav->walk();
        /*
